...
def predict_result_processing(output_data, interface_data, sampleWeight):
    ...
    try:
        prediction_data = predict_processor.predict_result_df(output_data, interface_data, sampleWeight)
        logging.info('Prediction data processed.')

        interface_data = predict_processor.predict_result_interface(prediction_data)
        logging.info('Interface data processed.')

        predict_processor.insert_into_data(prediction_data, interface_data)
...
async def upload_image(request: Request, information: str = Form(...), image_file: UploadFile = File(...)):
    ...
    try:
        ...
        if output_data is None or len(output_data) == 0:
            ...
            return JSONResponse(content={'error': 'Prediction process failed'}, status_code=500)
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=settings.predict_service_port)
